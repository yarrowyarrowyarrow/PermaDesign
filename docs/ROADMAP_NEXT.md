...
working across both documents. **Next free ID: F153.**
...
| **F152** | Two harebells side by side read as a pair of swapped common names | ✅ **V2.82** — plan: [`V2.82-the-harebells-were-not-swapped`](plans/V2.82-the-harebells-were-not-swapped.md). *“The common name for the harebells have been misapplied.”* They were not: *Campanula lasiocarpa* really is Alaska Harebell, and the error was on the other card — **`Campanula alaskana` is a name the flora does not accept**, and the catalogue's own VASCAN extract had said so since V2.80, recording `accepted_name: Campanula rotundifolia`. Swapping the two names, which is what the cards invite, would have made both wrong. **The generalisable part: a wrong scientific name is invisible until it is next to a similar one**, and a browse-by-name catalogue puts every synonym in it one page away from looking like this. So the same question was asked of all 422 rows, which takes six lines against a file already on disk and found **26 binomials that are not VASCAN's accepted name**. Not one problem: **5 were synonyms of a name already in the catalogue** — two pages, two ranges and two relationship sets for one plant — and **4 of V2.80's own 8 renames had landed on a rank the checklist does not carry** (`taxon_rank: species` on all four says the lookup never matched a subspecies), two of those colliding with species already here, which is how V2.80 shipped *Urtica gracilis* twice in two days. Three of the five pairs had already been diagnosed **by hand in the notes field of the row itself** (*“Marsh Hedge Nettle and Marsh Woundwort are two common names for the same species…”*) and one had got as far as an allowlist entry in `seed_flower_colour.KEEP` excusing its colour audit trip, which is a row apologised for in two files rather than a row with a colour problem. Merged, on the rule that the surviving row keeps the name already published at a URL somebody may have linked unless that name is the wrong one; *Shooting Star* is the one exception, retired for *Saline Shooting Star* because the bare name is the ambiguous one and *Desert Shooting Star* sits beside it. Seven **common** names were wrong too, found by the three signals available offline — a name that collides with another row's, a name that contradicts its own row's data, and a misspelling: *Boreale Oxytropis*, *Lyalls Penstemon*, *Yellow Pucoon*, *Rosy Pussytoes* on *Antennaria microphylla* (that is *A. rosea*, carried here as Pink Pussytoes, and `sprite_gallery.py` had independently bound the name to *A. rosea*), two bare names shared with a second row, and **White Camas on a plant whose own notes read “TOXIC — ALL PARTS… closely resembles edible blue camas”** — the whole historical hazard is that confusion, and the catalogue was doing it. Three tools: `rename_common_name.py` (new — the common name is the foreign key here and the scientific name is not), `remove_taxon.py --merge-into` now actually merges (field by field, larger derived entry kept, raw cache unioned, **15 duplicate edges dropped** that V2.80's yarrow merge shipped 135 of), and `rename_taxon.py` stopped clearing a sourced nativity when the new name **is** the archive's own answer for the old one, which is the case where there is nothing to wait for. Left alone and handed back: **18 recent recombinations** (*Galium boreale* → *Trichogalium boreale*, *Spartina* → *Sporobolus*, *Ledum* → *Rhododendron*…) — accepted by VASCAN, used by nobody the site is written for, and one decision about which nomenclature the site follows rather than eighteen, now allowlisted with a reason each in `data_quality.KNOWN_NOMENCLATURE` so a *seventeenth* fails the gate instead of joining them unnoticed; and four names that need a flora rather than an archive (*Carex rostrata*/*utriculata* possibly exchanged, “Indigenous Ricegrass” and “Rat Root”/“Bear Root” as P12 questions, *Viburnum opulus* publishing a native claim under a name that covers the European guelder-rose) |
